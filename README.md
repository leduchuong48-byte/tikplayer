...
> AList 媒体浏览、播放与整理的 Web 界面。

Tikplayer 2.4 是一个用于 AList 媒体源浏览、播放、下载和整理的 Web 界面，适合部署在 NAS 或其他自托管环境中。

## Why this tool?（为什么要做它）

常见文件浏览器更偏向通用文件管理，而不是围绕媒体播放流程组织操作。Tikplayer 把刷新源、随机播放、目录播放、下载、移动、删除和转码回退放到同一套 Web 界面里，减少在多个页面之间切换。

## 当前版本亮点

- 面向 AList 源的视频浏览与播放界面
