...
class FolderTargetDeleteModel(BaseModel):
    source: str
    path: str
...
def _normalize_folder_item(raw: dict) -> dict:
    name = str(raw.get("name") or "").strip()
    path = _normalize_media_path(str(raw.get("path") or "/"))
    ftype = str(raw.get("type") or "direct").strip().lower()
    if ftype not in {"direct", "group"}:
        ftype = "direct"
    icon = str(raw.get("icon") or ("📂" if ftype == "group" else "📁"))
    source = str(raw.get("source") or "")
    fid = str(raw.get("id") or "").strip() or uuid.uuid4().hex
...
def _load_folders_config() -> List[dict]:
    ...
    try:
        ...
        if not isinstance(data, list):
            data = []
        normalized = [_normalize_folder_item(item if isinstance(item, dict) else {}) for item in data]
        # rewrite if missing ids/fields migrated
        _save_folders_config(normalized)
        return normalized
    except Exception as e:
        print(f"Error: {e}")
        return []
...
def _save_folders_config(folders: List[dict]) -> None:
    os.makedirs(DATA_DIR, exist_ok=True)
...
async def get_folders():
    return _load_folders_config()


@app.post("/v1/folders")
async def save_folders(folders: List[dict]):
    try:
        normalized = [_normalize_folder_item(item if isinstance(item, dict) else {}) for item in folders]
        _save_folders_config(normalized)
...
@app.post("/v1/folders/item")
async def create_folder_item(payload: FolderConfigModel):
    folders = _load_folders_config()
    item = _normalize_folder_item(payload.dict())
    folders.append(item)
    _save_folders_config(folders)
    return {"status": "success", "item": item}


@app.put("/v1/folders/{folder_id}")
async def update_folder_item(folder_id: str, payload: FolderConfigModel):
    folders = _load_folders_config()
    idx = next((i for i, f in enumerate(folders) if f.get("id") == folder_id), -1)
    if idx < 0:
        raise HTTPException(status_code=404, detail="folder config not found")
    body = payload.dict()
    body["id"] = folder_id
    folders[idx] = _normalize_folder_item(body)
    _save_folders_config(folders)
    return {"status": "success", "item": folders[idx]}


@app.delete("/v1/folders/{folder_id}")
async def delete_folder_item(folder_id: str):
    folders = _load_folders_config()
    new_folders = [f for f in folders if f.get("id") != folder_id]
    if len(new_folders) == len(folders):
        raise HTTPException(status_code=404, detail="folder config not found")
    _save_folders_config(new_folders)
    return {"status": "success", "message": "配置已删除"}


@app.delete("/v1/folders-target")
async def delete_folder_target(payload: FolderTargetDeleteModel):
    source_name = payload.source
    dir_path = _normalize_media_path(payload.path)
    if source_name not in alist_instances:
    ...
    # 仅允许删除空目录（避免误删）
    files, err = await AlistHelper.list_files_with_error(
        shared_http_client or httpx.AsyncClient(),
        instance['url'].rstrip('/').replace('/dav', ''),
        token,
        dir_path,
    )
    if err:
        raise HTTPException(status_code=502, detail=err)
    if files:
        raise HTTPException(status_code=400, detail="目录非空，禁止删除")

    base_url = instance['url'].rstrip('/').replace('/dav', '')
    url = f"{base_url}/api/fs/remove"
    parent, name = os.path.split(dir_path.rstrip('/'))
    parent = parent or '/'
    body = {"dir": parent, "names": [name]}

    client = shared_http_client or httpx.AsyncClient()
    resp = await client.post(url, headers=AlistHelper.get_headers(token), json=body, timeout=15.0)
    data = resp.json()
    if data.get('code') != 200:
        msg = str(data.get('message') or 'delete dir failed')
...
def _action_response(operation: str, status: str, message: str, **kwargs):
    body = {
        "operation": operation,
        "status": status,
        "message": message,
...
async def _verify_delete_committed(instance: dict, file_path: str) -> tuple[bool, str, dict]:
    ...
    for delay in delays:
        ...
        last_exists = exists
        if exists is False:
            return True, "", {"target_exists": exists, "last_error": "", "wait_profile": list(delays)}
    if last_err:
        return False, f"删除后复核失败: {last_err}", {"target_exists": last_exists, "last_error": last_err, "wait_profile": list(delays)}
    return False, f"删除后复核失败: 文件仍存在 {target}", {"target_exists": last_exists, "last_error": "", "wait_profile": list(delays)}


async def _verify_move_committed(instance: dict, src_path: str, dst_path: str) -> tuple[bool, str, dict]:
    ...
    for delay in delays:
        ...
        if src_exists is False and dst_exists is True:
            return True, "", {"src_exists": src_exists, "dst_exists": dst_exists, "last_error": "", "wait_profile": list(delays)}

    if last_err:
        return False, f"移动后复核失败: {last_err}", {"src_exists": last_src_exists, "dst_exists": last_dst_exists, "last_error": last_err, "wait_profile": list(delays)}
    return False, (
        f"移动后复核超时(源是否存在={last_src_exists}, 目标是否存在={last_dst_exists})，"
        "可能仍在后台任务执行中，请稍后刷新确认"
    ), {"src_exists": last_src_exists, "dst_exists": last_dst_exists, "last_error": "", "wait_profile": list(delays)}
...
async def stream_video(request: Request, source: str, path: str):
    ...
    try:
        if method == "HEAD":
            upstream = await shared_http_client.request(
                "HEAD",
                raw_url,
            ...
            forward = {}
            for k in ["content-type", "content-length", "content-range", "accept-ranges", "etag", "last-modified", "cache-control"]:
                v = upstream.headers.get(k)
                if v:
                    forward[k] = v
            return Response(status_code=upstream.status_code, headers=forward)

        req = shared_http_client.build_request(
            "GET",
            raw_url,
            headers=proxy_headers,
        )
        upstream = await shared_http_client.send(req, stream=True, follow_redirects=True)
        ...
        return StreamingResponse(iter_upstream(), status_code=upstream.status_code, headers=forward)
    except httpx.HTTPError as e:
        raise HTTPException(status_code=502, detail=f"upstream stream proxy error: {e}")
...
async def delete_video(payload: dict):
    ...
    if source_name not in alist_instances:
        raise HTTPException(status_code=404, detail=_action_response("delete", "failed", "源不存在", failed_reason="source_not_found", path=file_path))
    instance = alist_instances[source_name]
    conf = instance.get('conf', {})
    token = instance['token']
    ...
    if pre_exists is False:
        raise HTTPException(status_code=404, detail=_action_response("delete", "failed", "源文件不存在", failed_reason="source_file_missing", path=file_path))
    ...
    if success:
        verified, verify_msg, verify_state = await _verify_delete_committed(instance, file_path)
        ...
        print(f"[delete][pending] path={file_path} verify_state={verify_state}")
        return _action_response(
            "delete",
            "pending",
            "已提交删除，稍后刷新确认",
    ...
    raise HTTPException(status_code=500, detail=_action_response("delete", "failed", "删除失败", failed_reason="upstream_delete_failed", path=file_path, detail=msg))
...
async def move_video(payload: dict):
    ...
    if src_err:
        raise HTTPException(status_code=502, detail=_action_response("delete", "failed", "源探测失败", failed_reason="probe_failed", path=file_path, detail=src_err))
    if src_exists is False:
        raise HTTPException(status_code=404, detail=_action_response("move", "failed", "源文件不存在", failed_reason="source_file_missing", src_path=src_path, dst_dir=dst_dir))
    ...
    for attempt in range(max_retries):
        ...
        if data.get('code') == 200:
            ...
            verified, verify_msg, verify_state = await _verify_move_committed(instance, src_path, dst_path)
            ...
            print(f"[move][pending] src={src_path} dst={dst_path} verify_state={verify_state}")
            return _action_response(
                "move",
                "pending",
                "已提交移动，稍后刷新确认",
        ...
        if 'sharing' in last_msg.lower() or 'share access' in last_msg.lower() or 'incompatible' in last_msg.lower():
            if attempt < max_retries - 1:
                ...
                continue
        break
    raise HTTPException(status_code=500, detail=_action_response("move", "failed", "移动失败", failed_reason="upstream_move_failed", src_path=src_path, dst_dir=dst_dir, detail=last_msg))

@app.post("/v1/fs/rename")
async def rename_file(payload: RenameModel):
    if payload.source not in alist_instances:
        raise HTTPException(status_code=404, detail=_action_response("rename", "failed", "源不存在", failed_reason="source_not_found", src_path=payload.path))
    ...
    try:
        ...
        data = resp.json()
        if data.get('code') != 200:
            raise HTTPException(status_code=500, detail=_action_response("rename", "failed", "重命名失败", failed_reason="upstream_rename_failed", src_path=payload.path, detail=data.get('message')))

        src_path = _normalize_media_path(payload.path)
        parent_dir = os.path.dirname(src_path) or "/"
        dst_path = _join_media_path(parent_dir, payload.new_name)
        verified, verify_msg, verify_state = await _verify_move_committed(instance, src_path, dst_path)
        if verified:
            return _action_response(
                "rename",
        ...
        print(f"[rename][pending] src={src_path} dst={dst_path} verify_state={verify_state}")
        return _action_response(
            "rename",
            "pending",
            "已提交重命名，稍后刷新确认",
        ...
        raise HTTPException(status_code=500, detail=_action_response("rename", "failed", "重命名异常", failed_reason="rename_exception", src_path=payload.path, detail=str(e)))
